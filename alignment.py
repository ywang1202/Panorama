...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        ...
        if m == eTranslate:
            ...
            match = [matches[i] for i in np.random.randint(len(matches), size=4)]
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        feat2 = np.array([x2, y2, 1])

        dist = np.linalg.norm(tran - feat2)
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        #TODO-BLOCK-BEGIN
        match = [matches[inlier_indices[i]] for i in range(len(inlier_indices))]
        M = computeHomography(f1, f2, match)
        #TODO-BLOCK-END
        #END TODO
